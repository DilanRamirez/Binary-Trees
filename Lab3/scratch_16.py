...
def buildDT(T, attributes, target, n_splits, goal_acc, min_size):
    ...
    T = decisionTreeNode(best_a, best_thr, left_child, right_child)
    return T

def NumNodes(dt):
    if decisionTreeNode is None:
        return 0
    return NumNodes(dt.left) + 1 + NumNodes(dt.right)
...
T = None
T = buildDT(T,train_data, train_target, 100, 0.95, 10)
